...
class TiwilioHandler:NSObject,LocalParticipantDelegate,RoomDelegate,CameraSourceDelegate,RemoteParticipantDelegate,VideoViewDelegate {
    ...
    @objc func toggleCamera(){
        if ( isVideoRendering ) {
            ...
            self.viewController.onGoingCallView.updateCallType(callType: "audio")
            viewController.setupAudioDevice()

            if UIDevice.current.userInterfaceIdiom == .phone {
                self.viewController.setControlsForVideo()
            }
        } else {
            ...
            self.viewController.onGoingCallView.updateCallType(callType: "video")
            viewController.setupAudioDevice()
    ...
    func roomDidConnect(room: Room) {
        ...
        if ( !self.viewController.isCallOutgoing && room.remoteParticipants.count < 1 ) {
            self.viewController?.showCallEndView()
    ...
    func roomIsReconnecting(room: Room, error: Error) {
        self.viewController.onGoingCallView.isReconnectingStatus(true)
        reconnectingStatus = true
    }
    
    func roomDidReconnect(room: Room) {
        self.viewController.onGoingCallView.isReconnectingStatus(false)
