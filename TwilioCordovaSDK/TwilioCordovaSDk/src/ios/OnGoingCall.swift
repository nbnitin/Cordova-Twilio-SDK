...
class OnGoingCall : UIView{
    ...
    func updateCallType(callType:String) {
    ...
    private func setCallDuration(){
        let minuteString = self.minutes < 10 ? "0\(self.minutes)" : "\(self.minutes)"
        let secondsString = self.seconds < 10 ? "0\(self.seconds)" : "\(self.seconds)"
        self.callDurationLabel.text = minuteString + ":" + secondsString
    }
    
    func isReconnectingStatus(_ status:Bool){
        if timer != nil {
            timer.invalidate()
        }
        if status {
            //reconnecting
            callDurationLabel.text = "Reconnecting"
        } else {
            //reconnected
            setCallDuration()
            trackCallDuration()
